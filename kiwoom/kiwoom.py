...
class Kiwoom(QAxWidget):
    def __init__(self):
        ...
        for code in self.portfolio_stock_dict.keys():
            ...
            self.dynamicCall("SetRealReg(QString, QStrievent_slotsng, QString, QString)", screen_num, code, fids, "1")
    ...
    def trdata_slot(self, sScrNo, sRQName, sTrCode, sRecordName, sPrevNext):
        ...
        if sRQName == "예수금상세현황요청":
            ...
            for i in range(rows):
                ...
                code_nm = code_nm.strip()
                order_no = int(order_no.strip()) #형변환
    ...
    def get_code_list_by_market(self, market_code):
        '''
        종목코드 리스트 받기
        #0:장내, 10:코스닥, 3:(ELW), 8:(ETP), 50:(KONEX), 4:뮤추얼펀드, 5(신주인수권), 6:리츠, 9:하이얼펀드, 30:K=OTU
    ...
    def chejan_slot(self, sGubun, nItemCnt, sFidList):
        ...
        if int(sGubun) == 0:  # 주문체결
            ...
            origin_order_number = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결'][
                '원주문번호'])  # 출력 : defaluse : "000000"
            ...
            order_price = self.dynamicCall("GetChejanData(int)",
                                           self.realType.REALTYPE['주문체결']['주문가격'])  # 출력: 21000
            ...
            order_gubun = self.dynamicCall("GetChejanData(int)",
                                           self.realType.REALTYPE['주문체결']['주문구분'])  # 출력: -매도, +매수
            order_gubun = order_gubun.strip().lstrip('+').lstrip('-')
            ...
            current_price = self.dynamicCall("GetChejanData(int)",
                                             self.realType.REALTYPE['주문체결']['현재가'])  # 출력: -6000
